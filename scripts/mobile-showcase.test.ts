...
it("maps capture scenes to the real application routes", () => {
  assert.equal(showcaseSceneUrl("threads", "environment-1"), "t3code-after-dark://");
  assert.equal(
    showcaseSceneUrl("environments", "environment-1"),
    "t3code-after-dark://settings/environments",
  );
  assert.equal(
    showcaseSceneUrl("thread", "environment-1"),
    "t3code-after-dark://threads/environment-1/remote-command-center",
  );
  assert.equal(
    showcaseSceneUrl("terminal", "environment-1"),
    "t3code-after-dark://threads/environment-1/remote-command-center/terminal?terminalId=term-1",
  );
  assert.equal(
    showcaseSceneUrl("review", "environment-1"),
    "t3code-after-dark://threads/environment-1/remote-command-center/review",
  );
});
