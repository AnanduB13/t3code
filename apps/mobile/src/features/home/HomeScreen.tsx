...
export function HomeScreen(props: HomeScreenProps) {
  ...
  const modePendingTasks = useMemo(
    () =>
      props.pendingTasks.filter(
        (pendingTask) =>
          isGeneralChatsProjectId(pendingTask.projectId) === (props.mode === "chats"),
      ),
    [props.mode, props.pendingTasks],
  );
  ...
  const v2PendingTasks = useMemo(
    ...
    [modePendingTasks, props.selectedEnvironmentId, v2ScopedProjectKeys, v2SearchQuery],
  );
  ...
  const hasMultipleConnections = Object.keys(props.savedConnectionsById).length > 1;
  const renderV2Item = useCallback(
    ({ item }: { item: ThreadListV2ListItem }) => {
