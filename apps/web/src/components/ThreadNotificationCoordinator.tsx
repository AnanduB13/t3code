  ...
  useEffect(() => {
    ...
    for (const thread of shell.snapshot.value.threads) {
      ...
      const kind =
        attention && attention !== prior.attention
          ...
          : completion !== null && (prior.completion === null || completion > prior.completion)
            ...
            : null;
      if (!kind) continue;
      const projectTitle = shell.snapshot.value.projects.find(
      ...
      if (
        ...
        const toastId = toastManager.add(
          stackedThreadToast({
            ...
            title: kind === "completion" ? thread.title : title,
            description:
              kind === "completion"
                ? projectTitle
                  ? `Completed in ${projectTitle}`
      ...
      if (
        ...
        continue;
      try {
        const notification = new Notification(`${APP_DISPLAY_NAME} · ${title}`, {
          body: projectTitle ? `${thread.title}\n${projectTitle}` : thread.title,
