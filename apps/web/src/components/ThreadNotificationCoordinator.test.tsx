...
vi.mock("@effect/atom-react", () => ({
  useAtomValue: () => ({
    ...
    snapshot: Option.some({
      projects: [{ id: "project-1", title: "After Dark" }],
      threads: [
        {
          id: "thread-1",
...
describe("thread notifications", () => {
  it("alerts once with system alerts off and opens the completed thread", async () => {
    ...
    expect(toast?.description).toBe("Completed in After Dark");
  ...
  it("keeps After Dark completion feedback for the active thread", async () => {
    ...
    expect(state.add).toHaveBeenCalledWith(
      expect.objectContaining({
        title: "Fix the login form",
        description: "Completed in After Dark",
      }),
    );
  });

  it.each(["blurred", "hidden", "archived", "disabled"])(
  ...
  it.each([
    ...
    expect(state.notification).toHaveBeenCalledWith(`${APP_DISPLAY_NAME} · ${title}`, {
  ...
  it("keeps system alerts when the app is in the background", async () => {
    ...
    await complete();
    expect(state.add).not.toHaveBeenCalled();
    expect(state.notification).toHaveBeenCalledWith(`${APP_DISPLAY_NAME} · Thread completed`, {
      body: "Fix the login form\nAfter Dark",
      tag: "env-1:thread-1",
      silent: true,
    });
