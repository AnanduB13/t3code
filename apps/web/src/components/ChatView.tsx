...
export default function ChatView(props: ChatViewProps) {
  ...
  const firstQueuedMessageId = activeThread?.queuedMessages[0]?.messageId;
    ...
    for (const message of optimisticUserMessages) {
      ...
      for (const attachment of message.attachments ?? []) {
        if (isImageAttachment(attachment) && attachment.previewUrl) {
          urls.set(attachment.id, attachment.previewUrl);
        }
      }
    }
    return urls;
  ...
  const timelineMessages = useMemo(() => {
    ...
    for (const message of queuedMessages) serverIds.add(message.messageId);
  ...
  useEffect(() => {
    const handler = (event: globalThis.KeyboardEvent) => {
      ...
      if (command === "thread.steerQueuedMessage") {
        if (!firstQueuedMessageId || !activeThreadId) return;
        event.preventDefault();
        event.stopPropagation();
        if (!event.repeat) {
          void steerQueuedMessage({
            environmentId,
            input: { threadId: activeThreadId, messageId: firstQueuedMessageId },
          });
        }
        return;
      }
  ...
  const onSend = async (
    ...
    const standaloneSlashCommand =
      ...
      composerReviewComments.length === 0
        ...
        : null;
    if (
      (standaloneSlashCommand === "default" || standaloneSlashCommand === "plan") &&
      multipleModelSelections === null
    ) {
  ...
  const panelToggleControls = (
    <PanelLayoutControls
      activityCenterActive={workspacePaneActive}
