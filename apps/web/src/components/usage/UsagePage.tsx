...
function isUsageWindowDays(value: number | string): value is UsagePagePreferences["windowDays"] {
...
export function UsagePage() {
  ...
  const chartSinceDay =
    windowDays === "all" ? (merged.daily[0]?.day ?? window.untilDay) : window.sinceDay;
  const days = useMemo(
  ...
  const selectWindow = (days: number | string) => {
    ...
    setWindowSelection({
      ...
      window:
        days === "all"
          ? makeAllTimeWindow()
          : makeWindow(days, undefined, days === 1 ? "hour" : "day"),
    });
  };
  ...
  const windowLabel =
    isPast24Hours && window.sinceTime !== undefined && window.untilTime !== undefined
      ...
      : `${formatDayShort(chartSinceDay)} to ${formatDayShort(window.untilDay)}`;
  const topbarContent = (
    <div className="grid w-full min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-x-3 gap-y-2 py-2 xl:flex">
      ...
      <div className="ms-auto hidden min-w-0 items-center justify-end gap-2 xl:flex">
        ...
        <ToggleGroup
          ...
          onValueChange={(next) => {
            const value = next[0];
            if (value) selectWindow(value === "all" ? "all" : Number(value));
          }}
        >
      ...
      <div className="col-span-2 ms-auto flex min-w-0 items-center justify-end gap-1 xl:hidden">
        ...
        <Select
          value={String(windowDays)}
          disabled={showingLimits}
          onValueChange={(value) => selectWindow(value === "all" ? "all" : Number(value))}
        >
          <SelectTrigger
            aria-label="Usage period"
            size="compact"
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col bg-background text-foreground">
        ...
        <ScrollArea className="min-h-0 flex-1">
          <WorkspacePageContainer width="wide">
            {!showingLimits ? <UsageDeviceStrip environments={selectedEnvironments} /> : null}
              ...
              <>
                ...
                <section className="flex flex-col gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-x-6 gap-y-4 py-1 md:grid-cols-3 xl:grid-cols-6">
                    <Metric label="Token cost" value={formatUsd(merged.costUsd)} />
                    <Metric
    ...
    <div className="border border-border text-xs">
      <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 border-b border-border px-3 py-2">
        ...
        <span className="text-muted-foreground">
          Includes T3 Code and terminal Codex, Claude Code, or Grok Build sessions saved on each
          environment
        </span>
      </div>
