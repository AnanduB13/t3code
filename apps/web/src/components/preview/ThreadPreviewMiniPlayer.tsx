...
function MiniPlayerShell({
  ...
  useLayoutEffect(() => {
    ...
    if (!element) return;
    const measure = () => {
      const next = measureLayout(element, wholeWindow ? null : composerOverlayElement);
  ...
  }, [composerOverlayElement, wholeWindow]);
  ...
  const handleKeyboard = (event: ReactKeyboardEvent<HTMLElement>, resize: boolean) => {
    const direction = {
      ArrowLeft: { x: -1, y: 0 },
      ArrowRight: { x: 1, y: 0 },
      ArrowUp: { x: 0, y: -1 },
    ...
    if (!direction || !frame || !container) return;
    event.preventDefault();
    event.stopPropagation();
    ...
    >
      {frame ? (
        <section
          ...
          className="group/player pointer-events-none absolute select-none"
          style={{
            left: frame.x,
            top: frame.y,
            width: frame.width,
        ...
        >
          ...
          >
            ...
            <div
              className="pointer-events-none absolute right-0 top-0 flex h-8 cursor-grab items-center gap-0.5 rounded-lg border border-border/80 bg-popover/92 p-0.5 opacity-0 shadow-lg/20 backdrop-blur-xl transition-opacity group-hover/player:pointer-events-auto group-hover/player:opacity-100 group-focus-within/player:pointer-events-auto group-focus-within/player:opacity-100 active:cursor-grabbing"
          ...
          {wholeWindow ? (
            <button
              type="button"
              aria-label="Move floating browser preview. Use arrow keys to move."
              className="pointer-events-auto absolute inset-0 z-[48] cursor-grab rounded-[inherit] touch-none focus-visible:ring-2 focus-visible:ring-ring active:cursor-grabbing"
              onPointerDown={(event) => beginGesture(event, null)}
              onPointerMove={handlePointerMove}
              onPointerUp={endGesture}
              onPointerCancel={endGesture}
              onKeyDown={(event) => handleKeyboard(event, false)}
            />
          ) : null}
          ...
          {RESIZE_HANDLES.map(({ direction, className }) => (
            <div
              ...
              tabIndex={direction === "southeast" ? 0 : -1}
  ...
  return wholeWindow ? createPortal(content, document.body) : content;
}
