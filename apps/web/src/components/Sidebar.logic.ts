...
>(
  threads: readonly T[],
  query: string,
  contentMatchKeysOrProjectLabel:
    | ReadonlySet<string>
    | ((thread: T) => string | null) = EMPTY_CONTENT_MATCH_KEYS,
  projectLabelForThread: (thread: T) => string | null = () => null,
): T[] {
  const normalizedQuery = query.trim().toLowerCase();
  if (normalizedQuery.length === 0) return [];
  const contentMatchKeys =
    typeof contentMatchKeysOrProjectLabel === "function"
      ? EMPTY_CONTENT_MATCH_KEYS
      : contentMatchKeysOrProjectLabel;
  const projectLabel =
    typeof contentMatchKeysOrProjectLabel === "function"
      ? contentMatchKeysOrProjectLabel
      : projectLabelForThread;
