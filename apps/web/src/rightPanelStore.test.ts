...
describe("rightPanelStore", () => {
  ...
  it("preserves dismissed After Dark devices across the id migration and allows reopening", () => {
    const target = {
      hostId: "host:one/local",
      deviceId: "phone:one/local",
      name: "Pixel",
      platform: "android",
    } as const;
    const legacyId = `device:${JSON.stringify([target.hostId, target.deviceId])}`;
    const migratedId = `device:${encodeURIComponent(target.hostId)}:${encodeURIComponent(target.deviceId)}`;
    const store = useRightPanelStore.getState();
    store.openDevice(refA, target);
    const persisted = {
      byThreadKey: Object.fromEntries(
        Object.entries(useRightPanelStore.getState().byThreadKey).map(([key, state]) => [
          key,
          {
            ...state,
            isOpen: false,
            ...
            dismissedDeviceSurfaceIds: [legacyId, migratedId],
          },
        ]),
      ),
    };
    ...
    // The same device id on another host is still eligible for automatic display.
    store.openDevice(refA, { ...target, hostId: "host:two/local" }, true);
    expect(
      selectThreadRightPanelState(useRightPanelStore.getState().byThreadKey, refA).surfaces,
    ).toHaveLength(1);
    store.openDevice(refA, target);
    const restored = selectThreadRightPanelState(useRightPanelStore.getState().byThreadKey, refA);
    expect(restored.surfaces).toHaveLength(2);
    expect(restored.activeSurfaceId).toBe(migratedId);
    expect(restored.dismissedDeviceSurfaceIds ?? []).toEqual([]);
  });
