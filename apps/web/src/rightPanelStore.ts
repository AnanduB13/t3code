...
// After Dark previously encoded host/device pairs as a JSON tuple. Closed tabs
// have no surface left to migrate, so their saved dismissal ids need decoding too.
function migrateDeviceSurfaceId(id: string): string {
  if (!id.startsWith("device:[")) return id;
  try {
    const target: unknown = JSON.parse(id.slice("device:".length));
    if (
      ...
      return `device:${encodeURIComponent(target[0])}:${encodeURIComponent(target[1])}`;
    }
  } catch {
    // Keep unknown ids intact; they must never dismiss a different device.
  }
  return id;
}
      ...
      ? Object.fromEntries(
          Object.entries(persistedState.byThreadKey as Record<string, ThreadRightPanelState>)
            ...
            .map(([threadKey, threadState]) => {
              ...
              const deviceIdMigrations = new Map<string, `device:${string}`>();
              const surfaces = Array.isArray(validThreadState?.surfaces)
                ? validThreadState.surfaces.flatMap<RightPanelSurface>((surface) => {
                    ...
                    if (surface.kind === "device" && surface.target) {
                      const id =
                        `device:${encodeURIComponent(surface.target.hostId)}:${encodeURIComponent(surface.target.deviceId)}` as const;
                      deviceIdMigrations.set(surface.id, id);
                      return [{ ...surface, id }];
                    }
                ...
                : [];
              const oldActiveSurfaceId = validThreadState?.activeSurfaceId;
              const rawActiveSurfaceId = oldActiveSurfaceId
                ? (deviceIdMigrations.get(oldActiveSurfaceId) ?? oldActiveSurfaceId)
                : null;
              // Collapse duplicate targets restored from old and new id formats.
                ...
                threadKey,
                {
                  ...
                  ...(Array.isArray(validThreadState?.dismissedDeviceSurfaceIds)
                    ? {
                        dismissedDeviceSurfaceIds: [
                          ...new Set(
                            validThreadState.dismissedDeviceSurfaceIds
                              .filter((id): id is string => typeof id === "string")
                              .map(migrateDeviceSurfaceId),
                          ),
                        ],
                      }
                    : {}),
