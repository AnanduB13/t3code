  ...
  const listQueuedMessageRows = SqlSchema.findAll({
    ...
    execute: () =>
      sql`
        ...
        ORDER BY thread_id ASC, queue_position ASC, rowid ASC
      `,
  });
...
pending_approval_requests AS (
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      ...
      return Option.some({
        ...
        lastVisitedAt: threadRow.value.lastVisitedAt ?? null,
  ...
  // Profile totals use one latest cumulative observation per thread. Aggregate
  // in SQLite so refreshing the profile never hydrates conversation histories.
  const getTokenUsageStats: NonNullable<ProjectionSnapshotQueryShape["getTokenUsageStats"]> = () =>
    sql<{
      readonly date: string;
      readonly tokens: number;
      readonly peakThreadTokens: number;
      readonly trackedThreads: number;
    }>`
      WITH usage_values AS (
        SELECT thread_id, created_at, sequence, activity_id,
          CASE
            WHEN json_type(payload_json, '$.totalProcessedTokens') IN ('integer', 'real')
              THEN json_extract(payload_json, '$.totalProcessedTokens')
            WHEN json_type(payload_json, '$.usedTokens') IN ('integer', 'real')
              THEN json_extract(payload_json, '$.usedTokens')
          END AS tokens
        FROM projection_thread_activities
        ...
        FROM usage_values WHERE tokens >= 0
      )
      SELECT substr(created_at, 1, 10) AS date,
        SUM(CAST(tokens AS INTEGER)) AS tokens,
        MAX(CAST(tokens AS INTEGER)) AS "peakThreadTokens",
        COUNT(*) AS "trackedThreads"
      FROM ranked_usage WHERE position = 1
      GROUP BY date ORDER BY date ASC
    `.pipe(
      Effect.map((rows) => ({
        lifetimeTokens: rows.reduce((sum, row) => sum + row.tokens, 0),
        peakThreadTokens: rows.reduce((peak, row) => Math.max(peak, row.peakThreadTokens), 0),
        trackedThreads: rows.reduce((sum, row) => sum + row.trackedThreads, 0),
        daily: rows.map(({ date, tokens }) => ({ date, tokens })),
      })),
      Effect.mapError(toPersistenceSqlError("ProjectionSnapshotQuery.getTokenUsageStats:query")),
    );
