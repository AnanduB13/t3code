...
describe("CheckpointReactor", () => {
  ...
  effectIt.effect("captures and reverts checkpoints from a nested Git workspace", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(harness.drain);
      expect(gitRefExists(repositoryRoot, checkpointBaselineRefForThreadTurn(threadId, 0))).toBe(
        true,
      );
      expect(yield* harness.nextReceipt).toMatchObject({
        type: "checkpoint.baseline.captured",
  ...
  effectIt.effect.each([
    ...
    Effect.gen(function* () {
      ...
      expect(gitRefExists(harness.cwd, checkpointBaselineRefForThreadTurn(threadId, 0))).toBe(
        timing === "between turns",
      );
