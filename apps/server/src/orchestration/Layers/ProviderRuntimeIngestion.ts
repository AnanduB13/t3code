import { takePendingVisualEvidence } from "../../visualEvidence/VisualEvidence.ts";
...
const make = Effect.gen(function* () {
  ...
  const projectionQueuedMessages = yield* ProjectionQueuedMessageRepository;
  ...
  const finalizeAssistantMessage = (input: {
    ...
    Effect.gen(function* () {
      ...
      if (input.hasProjectedMessage || hasRenderableText) {
        const attachments = isReasoning ? [] : yield* takePendingVisualEvidence(input.threadId);
        yield* orchestrationEngine.dispatch({
          type: isReasoning
  ...
  const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      ...
      const completingPendingTurn =
        isTerminalTurn && Option.isSome(pendingTurnStart) && eventTurnId
      ...
      const shouldApplyThreadLifecycle = (() => {
        ...
        switch (event.type) {
          ...
          case "turn.aborted":
            ...
            if (
              Option.isSome(pendingTurnStart) &&
              Option.isSome(completingPendingTurn) &&
              completingPendingTurn.value.state !== "running"
            ) {
              return false;
            }
      ...
      if (
        ...
        const queuedMessages = yield* projectionQueuedMessages.listByThreadId({
          threadId: thread.id,
        });
  ...
  const processDomainEvent = Effect.fn("processQueueDomainEvent")(function* (
    ...
    const thread = yield* resolveThreadRuntimeContext(event.payload.threadId);
    if (!thread || thread.session?.status === "running" || thread.session?.status === "starting") {
      return;
    }
    const queuedMessages = yield* projectionQueuedMessages.listByThreadId({ threadId: thread.id });
    if (queuedMessages.length === 0) {
      return;
    }
    yield* orchestrationEngine
