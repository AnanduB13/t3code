...
const serverConfigTestLayer = ServerConfig.layerTest(process.cwd(), fixtureCwdRoot).pipe(
...
routing.layer("ProviderServiceLive routing", (it) => {
  ...
  it.effect("appends attachment file paths to the turn input text", () =>
    Effect.gen(function* () {
      ...
      assert.include(mixedInput.input ?? "", '[Attached PDF "report.pdf" is saved at: ');
      ...
      assert.include(fileOnlyInput.input ?? "", '[Attached PDF "report.pdf" is saved at: ');
...
describe("agent browser access", () => {
  ...
  const startSessionWith = (
    ...
    options?: {
      ...
      readonly deviceSupport?: boolean;
    ...
    Effect.gen(function* () {
      ...
      const providerLayer = makeProviderServiceLive({
          ...
          Effect.sync(() => {
            ...
            return {
              config: {
                ...
                threadId: request.threadId,
                providerSessionId: `test:${request.threadId}`,
        ...
        Layer.provide(
          ServerSettings.ServerSettingsService.layerTest({
            enableDeviceSupport: options?.deviceSupport ?? true,
      ...
      yield* Effect.gen(function* () {
        ...
        options?.inspectSession?.(McpProviderSession.readMcpProviderSession(threadId));
        return session;
      }).pipe(Effect.provide(providerLayer));
  ...
  it.effect("keeps browser/computer access when device support is disabled", () =>
    Effect.gen(function* () {
      const threadId = asThreadId("devices-disabled");
      const issued = yield* startSessionWith(true, threadId, undefined, { deviceSupport: false });
      assert.deepEqual(issued, [
        { threadId, capabilities: ["computerUse", "preview", "pull-requests"] },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );

  it.effect(
    "captures the device service at construction and supplies its CLI to provider sessions",
    () =>
      startSessionWith({ browser: false, device: true }, asThreadId("device-cli-env"), undefined, {
        inspectSession: (config) => {
          assert.isTrue(config?.capabilities.has("device"));
          assert.include(
            config?.agentDeviceEnvironment?.PATH ?? "",
            NodePath.join("device", "bin"),
          );
          assert.equal(config?.agentDeviceEnvironment?.AGENT_DEVICE_NO_UPDATE_NOTIFIER, "1");
        },
      }).pipe(Effect.asVoid, Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("issues a credential with preview when agent browser access is on", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [
        { threadId, capabilities: ["computerUse", "device", "preview", "pull-requests"] },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
