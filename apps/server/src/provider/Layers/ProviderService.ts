...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  // Keep live heartbeats in memory rather than writing every content delta to SQLite.
  const runtimeActivity = new Map<ThreadId, { instanceId: ProviderInstanceId; at: string }>();
  ...
  const agentAccessSettings = Effect.fn("ProviderService.agentAccessSettings")(
    function* (threadId: ThreadId) {
      ...
      const environment = {
        ...
        device: settings.enableDeviceSupport && settings.enableAgentDeviceAccess,
      };
      ...
      return {
        ...
        device: settings.enableDeviceSupport && resolved.enableAgentDeviceAccess,
      };
    },
    Effect.catch((cause) =>
      Effect.logWarning(
  ...
  const processRuntimeEvent = (
    ...
    Effect.gen(function* () {
      ...
      const previousActivity = runtimeActivity.get(canonicalEvent.threadId);
      if (
        previousActivity?.instanceId !== source.instanceId ||
        Date.parse(canonicalEvent.createdAt) > Date.parse(previousActivity.at)
      ) {
        runtimeActivity.set(canonicalEvent.threadId, {
          instanceId: source.instanceId,
          at: canonicalEvent.createdAt,
        });
      }
      yield* increment(providerRuntimeEventsTotal, {
  ...
  const sendTurn: ProviderServiceMethod<"sendTurn"> = Effect.fn("sendTurn")(function* (rawInput) {
    ...
    const inputTextWithAttachments = yield* Effect.try({
      try: () =>
        buildProviderInputWithAttachments({
          ...(inputTextWithCitations !== undefined ? { text: inputTextWithCitations } : {}),
          attachments: resolvedAttachments,
        }),
      catch: (cause) => toValidationError("ProviderService.sendTurn", String(cause), cause),
    });
    ...
    // file is read.
    let inputTextWithAttachmentContext = inputTextWithAttachments;
  ...
  const listSessions: ProviderServiceMethod<"listSessions"> = Effect.fn("listSessions")(
    function* () {
      ...
      const activeSessions = sessionsByProvider
        ...
        .map((session) => {
          const activity = runtimeActivity.get(session.threadId);
          return activity?.instanceId === session.providerInstanceId &&
            Date.parse(activity.at) > Date.parse(session.updatedAt)
            ? { ...session, updatedAt: activity.at }
            : session;
        });
      const activeThreadIds = new Set(activeSessions.map((session) => session.threadId));
      for (const threadId of runtimeActivity.keys()) {
        if (!activeThreadIds.has(threadId)) runtimeActivity.delete(threadId);
      }
