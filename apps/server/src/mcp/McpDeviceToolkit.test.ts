...
it.effect("denies existing device credentials immediately after agent access is disabled", () => {
  const disabled = Layer.mock(DeviceService.DeviceService)({
    ...
    open: () => Effect.die("Disabled credentials must not boot devices"),
    close: () => Effect.die("Disabled credentials must not close devices"),
  });
  return Effect.gen(function* () {
    const server = yield* McpServer.McpServer;
    for (const name of ["device_list", "device_open", "device_screenshot", "device_close"]) {
      const result = yield* server
        .callTool({ name, arguments: { deviceId: "UDID-1", platform: "ios" } })
        .pipe(
          Effect.provideService(McpInvocationContext.McpInvocationContext, invocation(["device"])),
          Effect.provideService(McpSchema.McpServerClient, client),
        );
      ...
      if (name === "device_screenshot")
        expect(result.structuredContent).toMatchObject({
          error: { _tag: "DeviceToolUnavailableError" },
        });
      else
        expect(result.content).toEqual(
          expect.arrayContaining([
            expect.objectContaining({
              type: "text",
              text: expect.stringContaining("Agent device access is turned off"),
            }),
          ]),
        );
    }
