...
The After Dark edition uses `after-dark.png` as the single artwork layer in every project so development, nightly, and production builds keep the same identity. The generated platform assets preserve that supplied artwork, with only the classic macOS safe-area treatment applied to the macOS export.
