...
## After Dark runtime behavior

All six providers receive the T3 harness identity and Markdown image/video guidance without
changing the saved user prompt. Codex and Grok include the current model and explicit reasoning
effort. Codex prefers the collaborative browser when available and falls back to other available
live research tools when the desktop host cannot serve it. Browser, computer-use, device and pull
request capabilities are granted separately.

PDF and text attachment extraction supplements provider-native attachment handling. Attachment
paths remain server-owned and subject to the provider's permission policy. Antigravity retains
native fixed-choice question values and disables custom text when the runtime requires a choice.
