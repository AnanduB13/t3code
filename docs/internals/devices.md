...
MCP credentials add a `device` capability only when both device support and
agent device access are enabled. Existing browser access continues to grant
`preview` and `computerUse`. Provider adapters add the device CLI environment
without replacing their existing launch environment. Media tickets use After
Dark's existing cookie, bearer, and DPoP HTTP authentication helpers.
