...
Pinning does not prevent settlement. A settled pinned thread keeps its pin and pin order; unpinning it moves it to the settled section.
...
position until you move it again. For threads without a saved arrangement, creating a thread or sending a user prompt moves it to the top. Agent replies, tools, and reading a thread do not reorder it. The settled shelf
...
Automatic inactivity and merged-PR settlement are off by default in After Dark. Enable them in settings when desired. A closed pull request can also settle an idle
...
Opening a completed chat clears its **Done** badge and marks its notification as read across
devices connected to that environment. **Mark unread** and **Mark all read** sync too. Later
completions can appear unread again. Reading does not change sorting or settlement.

Use the sidebar project picker to filter threads, and **All projects** to clear it. Pins remain
visible across projects and environments. `Cmd/Ctrl+Shift+P` toggles the current thread's pin.
The header and sidebar menus retain **Unlink pull request** without changing the pull request itself.
