...
## Mobile setup and refresh

On mobile, open **Settings → Environments**, expand the environment, and choose **Set up
Antigravity**. The model picker also links to setup when an account has no usable models.
For remote Google sign-in, paste the full loopback return URL into **Complete sign-in** in
that same client. Refresh uses saved credentials and does not open a login page. A status
of not checked after restart does not require signing in again: starting a session verifies it.
Health checks, explicit model refreshes, and sign-out allow up to 90 seconds for slow hosts.

Project skills can live in `.gemini/skills`, `.agents/skills`, or the legacy `.agent/skills`,
in that precedence order. Running sessions update the model picker when native choices change.
