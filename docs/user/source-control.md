...
In web and desktop threads, click an image in **changed files** to preview its current workspace
contents. Text files open their turn diff. **Environment details** in the thread toolbar shows
changed lines, checkout and branch, upstream sync, pull request status and the next Git action.
Projects can use directories outside the server's initial working directory; review access follows
