...
describe("ServerSettings legacy assistant output", () => {
  it("buffers assistant responses by default", () => {
    expect(decodeServerSettings({}).enableLegacyTokenStreaming).toBe(false);
    expect(DEFAULT_SERVER_SETTINGS.enableLegacyTokenStreaming).toBe(false);
  });

  it("preserves an explicit token-streaming preference", () => {
    expect(
