...
describe("legacy usage compatibility", () => {
  it("shows older account windows without inventing reset credits", () => {
    const native = provider({ auth: { status: "authenticated", email: "test@example.com" } });
    const [adapted] = withLegacyUsageLimits(
      [native],
      [
        {
    ...
    expect(adapted?.auth).toEqual({
      status: "authenticated",
      email: "test@example.com",
      label: "pro",
    });
    expect(adapted?.usageLimits?.windows[0]).toEqual({
      id: "primary",
      label: "Session",
      kind: "session",
      usedPercent: 40,
    ...
    expect(adapted?.usageLimits?.resetCredits).toBeUndefined();
  });

  it("preserves streamed limits and credits when a legacy reader also exists", () => {
    const native = provider({
      usageLimits: {
